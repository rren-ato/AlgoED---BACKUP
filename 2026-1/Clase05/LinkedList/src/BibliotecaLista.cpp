...
void imprimir_r(const NodoLista * recorrido) {
    if (recorrido == nullptr) return;
    cout << setw(5)<<recorrido->elemento.numero;
    imprimir_r(recorrido->siguiente);
}

void imprimir_rec(const struct Lista &lista_numeros) {
    if (esListaVacia(lista_numeros)) {
        cout << "La lista esta vacia no se puede mostrar" << endl;
        return;
    }
    imprimir_r(lista_numeros.head);
}
