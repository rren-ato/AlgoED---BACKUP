...
//     while ((recorrido != nullptr) and (recorrido->elemento.ciudad != elemento.ciudad)) {
//         ultimo = recorrido;
//         recorrido = recorrido->abajo;
//     }
//
...
void imprimir(const struct Lista &listaTAD) {
    if (esListaVacia(listaTAD)) {
        cout << "EL grafo esta vacio no se puede mostrar" << endl;
    } else {
        ...
        while (vertices != nullptr) {
            ...
            cout << "[" << vertices->elemento.ciudad << "]: ";
            while (arista != nullptr) {
                cout << arista->elemento.ciudad << " (" << arista->elemento.ponderado << ")  ";
...
void insertarArista(struct Lista &listaVertices,
                    const struct ElementoLista &origen, const struct ElementoLista &destino) {
    struct NodoLista *vertice = listaVertices.inicio;
    //ACA recorro los vértices... busca el vértice
    while (vertice != nullptr) {
        if (vertice->elemento.ciudad == origen.ciudad) {
            break;
        }
...
// Función para buscar un vértice por su código (carácter)
struct NodoLista *buscarVertice(struct Lista &grafo, string ciudad) {
    struct NodoLista *vertice = grafo.inicio;
    while (vertice != nullptr) {
        if (vertice->elemento.ciudad == ciudad) {
            return vertice;
        }
